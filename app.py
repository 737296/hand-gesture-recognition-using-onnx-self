...
def main():
    ...
    # フィンガージェスチャー履歴---手指手势历史记录 #################################################
    gesture_history_length = 10
    finger_gesture_history = {}

    # 手のひらトラッキング用手のひら中心座標最新履歴---用于手掌追踪的手掌中心坐标的最新历史记录 #################################
    # {
    #   int(trackid1): [cx, cy],
    #   int(trackid2): [cx, cy],
    #   int(trackid3): [cx, cy],
    ...
    while True:
        ...
        # ============================================================= PalmDetection
        # ハンドディテクション - シングルバッチ処理---手部检测 - 单批处理
        ...
        # 手の検出件数がゼロになったらトラッキング用手のひら中心座標最新履歴を初期化---当检测到的手数为零时，初始化最新历史掌心坐标进行跟踪。
        if len(hands) == 0:
            palm_trackid_cxcy = {}
        # トラッキング用手のひら中心座標最新履歴とバウンディングボックスの検出順序紐づけリスト---用于跟踪和边界框检测顺序链接列表的手掌中心坐标的最新历史记录
        ...
        if len(hands) > 0:
            ...
            print("rects:",rects)
            # 回転角度をゼロ度に補正した手のひら画像の取得---获取旋转角度校正为零度的手掌图像
            ...
            for rect in rects:
                ...
                # 回転非考慮の領域の描画, オレンジ色の枠---绘制区域时不考虑旋转，橙色框
                rcx = int(rect[0])
                rcy = int(rect[1])
                half_w = int(rect[2] // 2)
                half_h = int(rect[3] // 2)
                ...
                保存手掌中心坐标的最新历史记录以进行手掌追踪
                    1. 从过去的历史中提取距离参考点最近的中心坐标
                    2. 如果距离超过100px，则会被识别为新手掌。
                    3、如果距离小于100px，则分配对应的trackid，并覆盖过去历史的中心坐标。
                """
                # 1. 過去履歴の中から基準点との距離が一番近い中心座標を抽出---从过去的历史中提取距离参考点最近的中心坐标
