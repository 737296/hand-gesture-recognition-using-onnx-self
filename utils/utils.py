...
def rotate_and_crop_rectangle(
    ...
    """Crop a rotated rectangle from a image. 从图像中裁剪旋转的矩形
    ...
    # Determine if rect is inside the entire image---确定矩形是否位于整个图像内
...
def keep_aspect_resize_and_pad(
    ...
    根据长边调整图像大小，使其适合指定的边界矩形，同时保持纵横比，并将短边填充到指定大小。
    ...
    resized_image: np.ndarray
        ...
        常规的resize 的padding算法有哪些(resize + padding):
        1,直接拉伸 (Stretch),忽略原始图像的宽高比.简单快速，但易失真。
        2,等比例缩放 + 黑边填充 (Letterbox Padding), 图像按比例缩放到目标尺寸的最大边，再通过在剩余空间填充（通常为黑色）使其匹配目标尺寸。常用，保留宽高比。
        3,等比例缩放 + 镜像填充 (Mirror Padding), 在缩放的基础上，使用原图的边缘像素值进行镜像填充。减少填充突兀感。
        4,等比例缩放 + 平铺填充 (Tile Padding),用图像的自身内容进行重复平铺填充。减少填充突兀感。
        5,等比例缩放 + 常量填充 (Constant Padding),使用固定颜色（如白色、黑色）进行填充。可控填充颜色。
        6,中心裁剪后填充 (Center Crop + Padding),将图像裁剪到接近目标尺寸的大小，然后进行填充。兼顾目标尺寸和图像内容。
    ...
    ash = resize_height / image_height # 224 / 480 = 0.4666
    asw = resize_width / image_width # 224 / 640 = 0.35
    if asw < ash:
        sizeas = (int(image_width * asw), int(image_height * asw)) # (224,168)
    else:
        sizeas = (int(image_width * ash), int(image_height * ash))
    resized_image = cv2.resize(image, dsize=sizeas) # 将原图片的最长边resize到224，另一条边按照原图片的比例缩放 (224,168) wh
    start_h = int(resize_height / 2 - sizeas[1] / 2) # 112 - 84 = 28
    end_h = int(resize_height / 2 + sizeas[1] / 2) # 196
    start_w = int(resize_width / 2 - sizeas[0] / 2) # 112 - 112 = 0
    end_w = int(resize_width / 2 + sizeas[0] / 2) # 224
    # [28:196,0:224,:]
    padded_image[start_h:end_h, start_w:end_w, :] = resized_image.copy() # Letterbox Padding
    # padded_image  上下或者左右补完黑色像素的图像 (224,224)
    # resized_image resize等比例缩放完的图像 (224,168)
