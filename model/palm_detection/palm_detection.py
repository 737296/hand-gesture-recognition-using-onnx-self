...
这是手掌上某个关键点（通常是手指或掌心等部位）的 X 坐标。该关键点的位置通常是由模型预测的。
kp0_y（第一个关键点的 Y 坐标）：

这是手掌上某个关键点的 Y 坐标，表示该关键点在图像中的垂直位置。
kp2_x（第二个关键点的 X 坐标）：

这是手掌上另一个关键点（通常是手指或掌心等部位）的 X 坐标。这个点常常用于与第一个关键点进行比较，计算旋转角度等信息。
kp2_y（第二个关键点的 Y 坐标）：

这是手掌上另一个关键点的 Y 坐标，表示该关键点在图像中的垂直位置。
